...
def gen_dist(project):
    fp = open(os.getcwd() + "\\" + project +"_all\\"+ "commit.txt", "r")
    commit_list = []
    for line in fp.readlines():
        commit_list.append(line)
    os.chdir(os.getcwd() + "\\" + project +"_all\\" + project)
    print "当前工作目录：", os.getcwd()
    os.system("git checkout " + commit_list[0])
    ...
    llen = len(commit_list)
    while now < 10:
        os.system("git checkout " + commit_list[now])
        print "第%d版本：" % now, os.system("git rev-parse HEAD")
    ...
    f1 = open(parent_path + "\\commit_tree.txt", "w")
    f2 = open(parent_path + "\\commit_str.txt", "w")
    for i in tree_dists:
        f1.write(str(i)+'\n')
    for i in str_dists:
        f2.write(str(i)+'\n')
